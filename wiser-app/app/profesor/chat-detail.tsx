...
export default function ChatDetail() {
  ...
  const { conversacionId, alumnoNombre } = useLocalSearchParams<{ conversacionId: string; alumnoNombre: string }>();
  const { usuario, loading: loadingUsuario } = useProfesor();
  ...
  const handleEnviar = async () => {
    ...
    if (!contenido || !usuario?.id) return;
    setEnviando(true);
    setTexto('');
    try {
      const nuevoMensaje = await enviarMensaje(Number(conversacionId), usuario.id, contenido);
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{alumnoNombre}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const esPropio = item.remitenteId === usuario?.id;
          return (
